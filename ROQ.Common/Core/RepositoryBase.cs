...
{
    // Perhaps I can use this DBcontext related design here.. 
                                ...
                                where TContext : DbContext, new()
    {
        ...
        public IEnumerable<TEntity> GetAll(Boolean RefreshContext = false)
        {
        ...
        public IEnumerable<TEntity> FindBy(Expression<Func<TEntity, bool>> predicate, Boolean RefreshContext = false)
        {
            if (RefreshContext == true)
                _context = new TContext();
        ...
        public TEntity FindBy_FirstOrDefault(Expression<Func<TEntity, bool>> predicate, Boolean RefreshContext = false)
        {
            if (RefreshContext == true)
                _context = new TContext();
            TEntity query = _context.Set<TEntity>().FirstOrDefault(predicate);
            return query;
        }

        public TEntity Get(long id, Boolean RefreshContext = false)
        {
            if (RefreshContext == true)
                _context = new TContext();
